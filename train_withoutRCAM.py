...
def train(train_loader, val_loader, model, criterion, optimizer, epoch, logger):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        avg_recall.update([r.data.item() for r in recall], input.size(0))
        avg_pre.update([p.data.item() for p in pre], input.size(0))
        avg_acc.update(acc.data.item(), input.size(0))
        loss = (loss - b).abs() + b
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
if __name__ == '__main__':
    ...
    train_loader = ImageDataLoader(mode='train', shuffle=True, batch_size=64, num_workers=16,
                                   img_path='data/train/')
    ...
    model = model.cuda()
    # model.load_state_dict(get_saved_model(save_dir='train_result'))
    # lp = LineProfiler()
    # lp_wrapper = lp(train)
